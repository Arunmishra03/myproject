...
const aqikey="a676d23a-12af-46bd-b7b5-94197c396e0c";
...
function showWeather(weatherData) {
  ...
  document.getElementById("realfeel").innerHTML = "Feels Like : "+weatherData.current.feelslike_c + "°C";
...
function updateWeather(j,forecastday){
  ...
  for(let i=12;i<=24;i++ ){
    ...
    document.getElementById(i).innerHTML=(forecastday[k].hour[j].time).slice(11);
    document.getElementById((2*i)).src=forecastday[k].hour[j].condition.icon;
    document.getElementById((4*i)).innerText=(forecastday[k].hour[j].temp_c) +"°C";
    j++;
    //forecastday[k].hour[j].temp_c
    // forecastday[k].hour[j].condition.icon
...
function showAqi(current){
  document.getElementById('pm2.5').innerHTML="PM2.5 : "+current.air_quality.pm2_5;
  document.getElementById('pm10').innerHTML="PM10 : "+current.air_quality.pm10;
...
function currentWeather() {
  ...
  const urlforecast = `https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${search.value}&days=7&aqi=yes&alerts=no`;
 
  getWeather(urlforecast).then(showWeather);
